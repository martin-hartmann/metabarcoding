...
# PRIMERS
primerF="CANCGATGAAGAACGYRG" # Tedersoo & Lindahl 2016 (https://doi.org/10.1111/1758-2229.12438)
primerR="CCTSCSCTTANTDATATGC" # Tedersoo & Lindahl 2016 (https://doi.org/10.1111/1758-2229.12438)
#primerF="AACTTTYRRCAAYGGATCWCT" # Taylor et al. 2016 (https://doi.org/10.1128/AEM.02576-16)
#primerR="AGCCTCCGCTTATTGATATGCTTAART" # Taylor et al. 2016 (https://doi.org/10.1128/AEM.02576-16)

# DATABASES
## the phiX genome sequences can be retrieved from https://www.ncbi.nlm.nih.gov/nuccore/NC_001422.1
## the UNITE database can be downloaded from https://unite.ut.ee/repository.php and ITS1 or ITS2 specific databases are generated by the following command
### ITSx -i in.fasta -o out.ITSx --complement F --preserve T --require_anchor HMM
PHIX="/databases/phix"
#TAX="/databases/unite.v80.ITS1_HMM.sintax" # rename to your file
TAX="/databases/unite.v80.ITS2_HMM.sintax" # rename to your file
...
vsearch --fastq_mergepairs 3.all.trim.R1.fastq --reverse 3.all.trim.R2.fastq --fastqout 4.all.merge.fastq --fastaout 4.all.merge.fasta --fastq_truncqual 7 --fastq_allowmergestagger --fastq_minovlen 30 --fastq_minmergelen 150 --threads $T # merge PE reads
...
ITSx -i 7.all.ASV_nochim.fasta -o 8.all.ASV_ITSx --complement F --preserve T --save_regions ITS2 --allow_single_domain F -N 2 --require_anchor HMM --cpu $T # verify target
...
vsearch --cluster_size 8.all.ASV_ITSx.fasta --centroids 8.all.OTU_ITSx.fasta --uc 8.all.OTU_ITSx.uc --id 0.97 --relabel OTU --sizeorder --sizein --sizeout --threads $T # cluster into OTUs
...
ITSx -i 8.all.OTU_ITSx.fasta -o 8.all.OTU_ITSx --complement F --preserve T --save_regions ITS2 --allow_single_domain F -N 2 --require_anchor HMM --cpu $T # verify target
vsearch --usearch_global 4.all.merge.fasta --db 8.all.ASV_ITSx.fasta --id 0.97 --maxhits 1 --maxaccepts 0 --uc 9.all.ASV_map.uc --matched 9.all.ASV_map.fasta --otutabout 9.all.ASV_map.txt --threads $T # map reads
vsearch --usearch_global 4.all.merge.fasta --db 8.all.OTU_ITSx.fasta --id 0.97 --maxhits 1 --maxaccepts 0 --uc 9.all.OTU_map.uc --matched 9.all.OTU_map.fasta --otutabout 9.all.OTU_map.txt --threads $T # map reads
...
vsearch --sintax 8.all.ASV_ITSx.ITS2.fasta --db $TAX --tabbedout 9.all.ASV_tax.sintax.txt --sintax_cutoff 0.8 --strand plus --threads $T # assign taxonomy
vsearch --sintax 8.all.OTU_ITSx.ITS2.fasta --db $TAX --tabbedout 9.all.OTU_tax.sintax.txt --sintax_cutoff 0.8 --strand plus --threads $T # assign taxonomy
...
vsearch --usearch_global 8.all.ASV_ITSx.ITS2.fasta --db $TAX --id 0.97 --maxaccepts 0 --maxrejects 0 --uc_allhits --uc 9.all.ASV_tax.lca.uc --lcaout 9.all.ASV_tax.lca.txt --threads $T # assign taxonomy with LCA
vsearch --usearch_global 8.all.OTU_ITSx.ITS2.fasta --db $TAX --id 0.97 --maxaccepts 0 --maxrejects 0 --uc_allhits --uc 9.all.OTU_tax.lca.uc --lcaout 9.all.OTU_tax.lca.txt --threads $T # assign taxonomy with LCA
